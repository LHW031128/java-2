...
# 익명 클래스로 이벤트 리스너 작성
 ...
 new익명클래스의 슈퍼클래스이름(생성자인자들){

 익명 클래스의 멘버 구현

 };
 - 간단한 리스너의 경우 익명 글래스 사용 추천
...
1. 유니코드 기의 특징
  국제 산업 표준
  전 세계의 문자를 검퓨터에서 일관되게 표현하기 위한 코드 체계
  문자들에 대해서만 키 코드 값 정의 : A~Z, a~z, 0~9, !, @, $ 등
2. 문자가 아닌 키 경우에는 표준화된 키 코드 값 없음
  <Function> 키, <Home> 키, <Up> 키, <Delete> 키, <Control> 키, <Shift> 키, <Alt>
  등은 플랫폼에 따라 키 코드 값이 다를 수 있음
...
# 다른 컴포넌트의 메소드
 1. 앞에서 살펴본 바와 같이 JComponent의 메소드를 상속 받기 때문에 이 장에서 설명하는 컴포넌트의 사용법의 거의 동일합니다

# 나머지 챔터의 내용 정리
나머지 4개의 챔터의 내용은 다음과 같습니다. 필요에 따라서 선택적으로 학습하면 됩니다

1. 11장 그래픽 : paintComponent() 활용하여 app에 직접 그림을 그리는 방법
2. 12장 자바 스레드 기초 : 멀티 태스킹과 스레드의 개념
3. 13장 입출력 스트림과 파일 입출력 : 자바의 입출력 스트림. 텍스트와 바이너리 file 입출력
4. 14장 자바 소켓 프로그래밍 : 소켓 통신에 대한 이해. 인터넷통신

5. 복습이 필요하다면 4장 클래스와 객체, 5장 상속, 8장 자바 GUI 스윙 기초, 9장 자바의 이벤터 처리를 다시 한번 학습 하세요.
